...
public class AccountRepositoryImpl extends AbstractRepositoryImpl<Account, String> implements iuh.fit.se.group1.repository.interfaces.AccountRepository {
    ...
    public Account findByUsername(String username) {
        ...
        return callInTransaction(emf ->
                emf.createQuery(query, Account.class).setParameter("username", username).getResultList()
                        .stream().findFirst().orElse(null)
        );
    }

    @Override
    public boolean updatePassword(String accountId, String newHashedPassword) {
        String query = """
                UPDATE Account a
        ...
        return callInTransaction(emf ->
                emf.createQuery(query, Account.class).setParameter("accountId", accountId).setParameter("newPass", newHashedPassword).executeUpdate() > 0
        );
